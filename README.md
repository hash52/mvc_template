...
# 概要
既存のフレームワークを使わずにWEBアプリを開発できる学習用教材です。
LaravelやRailsを使って学習するデメリットは、それらが何故動くのかが見えづらい点にあります。
（例、コントローラークラスを定義していますが、それらは一体いつインスタンス化されているのでしょうか・・？)

このフレームワークは、PHPの基本的な文法さえわかれば全ての処理を追えるよう意識して書かれています。
従来の教材ではブラックボックス化されがちだった部分を根本から学びましょう！


# 環境構築
ではさっそく、手元にダウンロードして動かしてみましょう
...
インストールしたMAMPのアプリ(象のアイコンです)を開いてください
...
## 4. ここまで来ればゴールは目前です。１でダウンロードしたソースコードの、mvc_template/public/index.phpを編集しましょう
まずは、13行目、`$mvc_template_path`の中身を編集します。`$mvc_template_path`の値は、１でメモしたパスです

次に32行目、`$connInfo`の情報を確認します
MAMPの「Preferences」から、「Ports」タブを開き「MySQL Port」にある値を、index.php35行目のport番号として記述しましょう
![mvc_mamp_port](https://user-images.githubusercontent.com/17327364/70988581-7897f880-2105-11ea-8e8e-c5e4575aca9b.png)

(index.php修正後の例。【】に注目)
```
$mvc_template_path = '/Users/tanakatarou/workspace/mvc_template/'; // <= 【自分のパソコンにあるmvc_templateの場所】
...
Laravelを使った学習と大きく異なる点は、継承する親クラスの実装まで読めるということです!
```
(ROOT)/
　　∟library/
　　　　∟mvc/
...
.htaccessの設定で、全てのパスはこのindex.phpに集まるようになっています（.htaccessはサーバの設定ファイルです。今はまだわからなくても大丈夫です）
まずは、index.phpからコードを読み進めてみましょう！

このフレームワークには、[足りない機能](https://github.com/hash52/mvc_template/issues)が山程あります！！！！
# コードの改修を絶賛募集中です。
[改修の方法ですが、このリポジトリをforkして、コードを記述したら、プルリクを投げてください](https://qiita.com/aipacommander/items/d61d21988a36a4d0e58b)
